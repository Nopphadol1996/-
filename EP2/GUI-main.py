...
# DATABASE
from db_maintenance import * # import file DB ที่เขียนแยก
...
def save():
    ...
    v_number.set('')
    v_tel.set('')
    update_table() # เมื่อกด Save ให้ ตาราง update data
...
def EditPage_mtworkorder(event=None):
    ...
    t_problem = op[4]
    t_number = op[5]
    t_tel = '0{}'.format(op[6]) # Bug เลข0หาย ดังนั้นจึงใช้ .fomath เข้ามาช่วย
    ...
    v_name2.set(t_name) # เอาค่าเดิมมาใส่
    ...
    v_department2.set(t_department) # เอาค่าเดิมมาใส่
    ...
    v_machine2.set(t_machine)  # เอาค่าเดิมมาใส่
    ...
    v_problem2.set(t_problem)  # เอาค่าเดิมมาใส่
    ...
    L.place(x=30,y=250)
    v_number2 =StringVar()
    v_number2.set(t_number)  # เอาค่าเดิมมาใส่
    ...
    L.place(x=30,y=300)
    v_tel2 =StringVar()
    v_tel2.set(t_tel)  # เอาค่าเดิมมาใส่
...
mtworkorderlist.bind('<Double-1>',EditPage_mtworkorder) # double click เพื่อแก้ไขข้อมูล
...
def Delete_mtworkorder(event=None):
    ...
    if check == True: # คุณต้องการลบไหม
        delete_mtworkorder(tsid)
        update_table()

mtworkorderlist.bind('<Delete>',Delete_mtworkorder) # กดปุ่ม Delete  เพื่อลบข้อมูล
